with p as (
    ...
    from dw.public_jaffle_shop.payments
    where status <> 'fail'
    group by 1
),
...
paid_orders as (
    ...
    from dw.public_jaffle_shop.orders as orders
    left join p on orders.id = p.order_id
    left join dw.public_jaffle_shop.customers as c on orders.user_id = c.id
),
...
customer_orders as (
    ...
    from dw.public_jaffle_shop.customers as c
